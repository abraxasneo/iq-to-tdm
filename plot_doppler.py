#!/usr/bin/env python3
"""Porównanie Dopplera: nasz TDM (Welch) vs CAMRAS single-FFT, 2022-12-01 21:42 UTC."""
...
with open('examples/generated_small.tdm') as f:
    for line in f:
        ...
        if m:
            ...
            if t:
                our_times.append(t)
                our_freqs.append(float(m.group(2)))   # już względem FREQ_OFFSET

# ── Wczytaj CAMRAS single-FFT doppler_20221201.txt ───────────────────────────
    ...
    for line in f:
        ...
        try:
            ...
            if clip_start <= t <= clip_end:
                sfft_times.append(t)
                sfft_freqs.append(hz - CENTER)   # offset od center
        except ValueError:
            pass

print(f"Nasz TDM (Welch):     {len(our_times)} pomiarów → {our_freqs}")
print(f"CAMRAS single-FFT:    {len(sfft_times)} pomiarów w oknie 21:40–21:46 UTC")
if sfft_freqs:
    valid = [f for f in sfft_freqs if abs(f) < 200_000]
    print(f"  zakres: {min(valid):+.0f} … {max(valid):+.0f} Hz  (po odrzuceniu skoków)")

# ── Wykres ───────────────────────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(11, 5))
fig.suptitle('Artemis I Doppler — 2022-12-01 ~21:42 UTC, CAMRAS Dwingeloo\n'
             'Porównanie: Welch (nasz TDM) vs single-FFT (CAMRAS reference)',
             fontsize=12, fontweight='bold')

# Single-FFT — szare kropki (surowe, hałaśliwe)
ax.scatter(sfft_times, sfft_freqs, s=6, color='silver', alpha=0.5,
           label='CAMRAS single-FFT (surowe, 1 s)', zorder=2)

# Single-FFT — mediana krocząca 5s dla czytelności
if len(sfft_times) >= 5:
    import statistics
    med_times, med_freqs = [], []
    ...
    ax.plot(med_times, med_freqs, color='darkorange', linewidth=1.5,
            label='CAMRAS single-FFT (mediana krocząca 5 s)', zorder=3)
...
ax.set_xlabel('UTC')
ax.set_ylabel('Doppler offset od 2216.500 MHz [Hz]')
